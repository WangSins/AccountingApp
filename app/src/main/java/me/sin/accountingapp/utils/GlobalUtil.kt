        ...
        set(context) {
            ...
            for (i in costTitle.indices) {
                if (costRes.size < costTitle.size) {
                    val res = CategoryResBean().also {
                        it.title = costTitle[i]
                        it.resBlack = costIconResBlack[i]
                        it.resWhite = costIconRes[i]
                    }
                    costRes.add(res)
                }
            }
            ...
            for (i in earnTitle.indices) {
                if (earnRes.size < earnTitle.size) {
                    val res = CategoryResBean().also {
                        it.title = earnTitle[i]
                        it.resBlack = earnIconResBlack[i]
                        it.resWhite = earnIconRes[i]
                    }
                    earnRes.add(res)
                }
            }
