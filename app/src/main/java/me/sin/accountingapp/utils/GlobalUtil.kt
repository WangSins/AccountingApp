...
class GlobalUtil {
    ...
    lateinit var databaseHelper: RecordDatabaseHelper
    var context: Context? = null
        set(context) {
            field = context
            databaseHelper = context?.let { RecordDatabaseHelper(it, RecordDatabaseHelper.DB_NAME, null, 1) }!!

            for (i in costTitle.indices) {
                if (costRes.size < costTitle.size) {
                    val res = CategoryResBean()
